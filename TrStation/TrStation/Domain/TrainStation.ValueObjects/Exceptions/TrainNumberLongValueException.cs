...
namespace TrainStation.Domain.TrainStation.ValueObjects.Exceptions
{
    internal class TrainNumberLongValueException(string trainNumber, int maxLength)
        : ArgumentException($"Number of train length {trainNumber} greated than maximum allowed length (допустимая длина) {maxLength}") // Было FormatException. Исключение, которое возникает в случае, если формат аргумента недопустим или строка составного формата построена неправильно. Наследование  Object ==> Exception ==> SystemException ==> FormatException
